...
samples <- tribble(
  ~sample_id, ~sample_type, ~polymer_type, ~known_mw, ~known_dispersity, ~dn_dc, ~extinction_coef,
  # Polystyrene standards (narrow dispersity, UV-active)
  "PS-1K", "standard", "polystyrene", 1000, 1.05, 0.185, 1.2,
  "PS-10K", "standard", "polystyrene", 10000, 1.03, 0.185, 1.2,
  "PS-50K", "standard", "polystyrene", 50000, 1.02, 0.185, 1.2,
  "PS-100K", "standard", "polystyrene", 100000, 1.01, 0.185, 1.2,
  "PS-500K", "standard", "polystyrene", 500000, 1.01, 0.185, 1.2,
  # PMMA samples (broader dispersity, weak UV)
  "PMMA-Low", "sample", "pmma", 25000, 1.8, 0.084, 0.1,
  "PMMA-Med", "sample", "pmma", 75000, 2.0, 0.084, 0.1,
  "PMMA-High", "sample", "pmma", 200000, 2.2, 0.084, 0.1,
  # PEG samples (no UV absorption)
  "PEG-5K", "sample", "peg", 5000, 1.1, 0.135, 0.0,
  "PEG-20K", "sample", "peg", 20000, 1.15, 0.135, 0.0,
  # Copolymers (variable UV)
  "Copoly-A", "sample", "copolymer", 40000, 1.5, 0.150, 0.6,
  "Copoly-B", "sample", "copolymer", 80000, 1.7, 0.160, 0.8
)
...
generate_chromatogram <- function(sample_row, time_points) {
  ...
  conc_profile <- generate_peak(time_points, peak_time, dispersity, intensity = 1)
  ...
  uv_signal <- add_drift(uv_signal, time_points, baseline_drift * max(ext_coef, 0.1))
...
sec_triple_detect <- chromatograms |>
  left_join(
    select(samples, sample_id, sample_type, polymer_type, known_mw,
           known_dispersity, dn_dc, extinction_coef),
    by = "sample_id"
  ) |>
  select(
    sample_id, sample_type, polymer_type,
    elution_time, ri_signal, uv_signal, mals_signal,
    known_mw, known_dispersity, dn_dc, extinction_coef
  )
