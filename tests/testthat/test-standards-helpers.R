...
test_that("match_standards correctly matches PS standards", {
  ...
  expect_equal(result$matched_standard, c("PS A", "PS B", "PS C", "PS D", "PS 1683"))
})
...
test_that("match_standards includes all with return_all = TRUE", {
  ...
  expect_equal(nrow(result), 3)
  expect_true(is.na(result$matched_standard[result$sample_name == "Unknown Sample"]))
})
...
test_that("auto_detect_standards uses specified name_col", {
  ...
  result <- auto_detect_standards(data, name_col = "my_names", add_columns = TRUE)
