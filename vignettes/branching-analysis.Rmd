...
Visualize structure-MW relationships with Rg-MW plots. The slope reveals polymer conformation:
...
ggplot(plot_data, aes(x = mw, y = rg, color = topology)) +
  geom_point(alpha = 0.3, size = 1) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1) +
  scale_x_log10(labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  scale_y_log10() +
  scale_color_manual(
    values = c("linear" = "#2E86AB", "branched" = "#A23B72", "star" = "#F18F01")
  ) +
  labs(
    x = expression(log[10](M[w])),
    y = expression(log[10](R[g]~"(nm)")),
    color = "Topology",
    title = "Conformation Plot: Rg vs Molecular Weight",
    subtitle = "Slope ≈ 0.5-0.6 (linear), < 0.5 (branched/compact)"
  ) +
  theme_minimal() +
  theme(legend.position = "bottom")
```
