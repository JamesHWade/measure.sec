...
### measure_list: Multiple Chromatograms

A **`measure_list`** is a list column containing multiple `measure_tbl` objects. This is how you store chromatograms alongside sample metadata in a tidy format:

```
┌──────────────────────────────────────────────────────────────┐
│  sample_id   known_mw   dn_dc       ri_signal               │
├──────────────────────────────────────────────────────────────┤
...
3. **Multiple detectors**: Store RI, UV, and MALS signals as separate columns in the same data frame
4. **Metadata preservation**: Sample properties like `dn_dc` or `known_mw` travel with the chromatogram

## Example Dataset

The package includes `sec_triple_detect`, a synthetic multi-detector SEC dataset. This is a good starting point for learning the workflow before analyzing your own data.

```{r data}
# Load the example dataset
data(sec_triple_detect, package = "measure.sec")

# View the structure - notice the measure_list columns (ri_signal, uv_signal, etc.)
...
- **Three detector signals**: RI, UV, and MALS (stored as measure_list columns)
...
```{r samples}
# View the sample types in the dataset
# Each row represents one injection with a complete set of detector signals
sec_triple_detect |>
...
# View the data structure - notice ri_signal is a measure_list
ps_sample |>
...
rec <- recipe(~., data = ps_sample) |>
  ...
  # dn/dc converts refractive index change to concentration
...
Use `step_sec_mw_averages()` to calculate these:
...
rec <- recipe(~., data = ps_sample) |>
  ...
  step_sec_baseline(measures = "ri") |>
  step_sec_ri(measures = "ri", dn_dc_column = "dn_dc") |>
  # Calculate MW averages
  # mw_column tells the step where to find molecular weight values
  # (from calibration or known standards)
...
rec <- recipe(~., data = ps_sample) |>
  ...
  # Now we can calculate MW averages using the calibrated MW values
